...
public class ClassController {
	...
	public String Get_TeacherClass(Model model, String classid) {
		...
		if (session_manage.getSession_mail() == null) {
			...
			socketSessionidForm.setTeacher_sessionid(session_id);
	...
	public String Post_EnterProcess(String classid, String time_period, RedirectAttributes redir) {
		try {
			...
			timetabletimeEntity = timetabletimeRepository.SearchIdByPeriodAndSchoolCode(int_time_period,
					classEntity.get().getSchool().getSchoolcode());
	...
	// 入室ログ、授業画面に遷移
	@GetMapping(value = "/enterprocess")
	public String Get_EnterProcess(String classid, String time_period, RedirectAttributes redir) {
		try {
			// 学生情報を取得
			String session_mail = session_manage.getSession_mail();
			StudentEntity studentEntity = new StudentEntity();
